...
This document defines how the API transforms a transaction into a fraud detection vector. It covers the vectorization (the 14 dimensions) and the normalization rules. The vector search uses that vector to find, in the reference dataset, the transactions most similar to the one that just arrived, and from there decide whether the new transaction is fraudulent.

## Flow Overview

The flow below shows, with a real example of a legitimate transaction, the step by step the API follows to decide on a transaction. In this case, a customer makes a low-value purchase at a merchant they already know, close to home.
...
Transactions need to be transformed into 14-position vectors, following the order and normalization rules below.
...
Once the vector is ready, the API should:
...
To measure "proximity" between two vectors, the API uses **Euclidean distance** over the 14 dimensions.

## Example of a Fraudulent Transaction

To contrast with the legitimate case, see how a fraudulent transaction looks: a high value, far from home, at an unknown merchant, with no previous transaction history.
